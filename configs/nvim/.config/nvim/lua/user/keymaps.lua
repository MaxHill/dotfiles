...
-- Keymaps
vmap("<leader>ef", require("user.extract-file").extract_selection_to_file, "Extract current selection to a new file")
nmap("<leader><space>c", function()
	require("user.open_assets").toggle(".*css$")
end, "Open css files in current dir in a split")
nmap("<leader><space>t", function()
	require("user.open_assets").toggle(".*ts$")
end, "Open ts files in current dir in a split")
...
nmap("<leader>ss", require("sg.extensions.telescope").fuzzy_search_results, "[S]earch [S]ourcegraph")
