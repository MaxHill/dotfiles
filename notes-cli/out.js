...
// index.ts
var notes_cli_exports = {};
__export(notes_cli_exports, {
...
var createMeetingNote = () => {
  const bulletIndicator = "3b95a551-26f9-4bc0-bfbc-33b1ecb8155f";
  const result = runCommand("icalBuddy", ["-b", bulletIndicator, "-ea", "eventsToday"]);
  const meeting = result.split(bulletIndicator)[2];
  console.log(meeting.split(/\r?\n/));
  const obj = meeting.split(/\r?\n/).reduce((acc, line) => {
    const indentation = line.replace(/^(\s*).*$/, "$1").length;
...
var createNote = (filename, { folder, content }) => {
  if (!filename) {
    console.error("Specify a filename");
    return;
  }
  const newNote = pipe(createNoteFromPath, addMdToFilename, createFile, openNote);
  newNote(baseDir + addTrailingSlash(folder) + filename, content);
};
var runCommand = (cmd, args = [], childProcess = cp) => {
  const process2 = childProcess.spawnSync(cmd, args, { stdio: "pipe" });
  if (process2.status) {
    throw new Error(process2.stderr.toString());
  }
...
program.command("new [filename]").description("Create a new note").option("-f, --folder <folder>", `Sub folder of basedir (${baseDir})`, "").option("-c, --content <content>", `Text to be added to file`, "").action(createNote);
program.command("meeting-note").description("Create a new meeting note").action(createMeetingNote);
program.parse();
module.exports = __toCommonJS(notes_cli_exports);
