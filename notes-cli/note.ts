...
export const createNoteFromPath = (filePath, content = '') => ({
  content,
  ...splitFileAndPath(filePath),
})

export const addMdToFilename = (note: INote) => ({
  ...note,
...
export const openNote = (note: INote) => {
  ...
  console.log(filePath) // This can be piped to whatever program you want
...
export const addTrailingSlash = (str: string) =>
  str.length < 1 || str.endsWith('/') ? str : `${str}/`
